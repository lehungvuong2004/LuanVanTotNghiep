...
export const Sidebar = ({ isCollapsed }: { isCollapsed: boolean }) => {
  ...
  // Lấy danh sách menu (navItems) dựa theo vai trò
  let navItems: any[] = [];
  if (role === ROLES.ADMIN) {
    navItems = NAV_ITEMS_BY_ROLE[ROLES.ADMIN];
  } else if (role === ROLES.OPERATOR) {
    navItems = NAV_ITEMS_BY_ROLE[ROLES.OPERATOR];
  } else if (role === ROLES.HELPER) {
    navItems = NAV_ITEMS_BY_ROLE[ROLES.HELPER];
  }

  // Lấy theme màu sắc dựa theo vai trò (Mặc định là Helper theme)
  let theme = THEME_BY_ROLE[ROLES.HELPER];
  if (role === ROLES.ADMIN) {
    theme = THEME_BY_ROLE[ROLES.ADMIN];
  } else if (role === ROLES.OPERATOR) {
    theme = THEME_BY_ROLE[ROLES.OPERATOR];
  }
    ...
    <aside className="w-full h-full bg-white dark:bg-slate-800 flex flex-col justify-between overflow-hidden">
      ...
      <div className={`px-4 mb-4 mt-4 shrink-0 ${isCollapsed ? "flex flex-col items-center" : ""}`}>
        <div className="flex items-center gap-3">
          ...
          {!isCollapsed && (
            <div>
              <h1 className={`font-bold text-base ${theme.brandText} leading-tight truncate max-w-[125px]`}>Gia Đình Việt</h1>
      ...
      <div className="shrink-0 border-t border-slate-100 dark:border-slate-700/50 pt-3 bg-white dark:bg-slate-800">
        <div className="px-3 mb-2">
          ...
          >
            <img alt="Profile" className="w-9 h-9 rounded-full object-cover shadow-xs shrink-0" src={avatar} title={isCollapsed ? `${fullName} (${email})` : undefined} />
            {!isCollapsed && (
