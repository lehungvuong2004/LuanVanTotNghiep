...
export const Notifications = () => {
  ...
  const renderTitleInput = () => (
    <div className="space-y-1.5">
      ...
      <input
        ...
        disabled={loading}
        className="w-full px-4 py-2.5 rounded-xl border border-slate-250 dark:border-slate-650 bg-slate-50 dark:bg-slate-900 text-slate-855 dark:text-slate-100 focus:outline-hidden focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all text-sm"
        required
      />
    </div>
  );
  ...
  const renderMessageInput = () => (
    <div className="space-y-1.5">
      ...
      <textarea
        ...
        rows={4}
        disabled={loading}
        className="w-full px-4 py-2.5 rounded-xl border border-slate-250 dark:border-slate-650 bg-slate-50 dark:bg-slate-900 text-slate-855 dark:text-slate-100 focus:outline-hidden focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all text-sm resize-none"
        required
      />
    </div>
  );
    ...
    <div className="space-y-1.5">
      ...
      <div className="relative">
        <select
          ...
          disabled={loading}
          className="w-full appearance-none px-4 py-2.5 rounded-xl border border-slate-250 dark:border-slate-650 bg-slate-50 dark:bg-slate-900 text-slate-855 dark:text-slate-100 focus:outline-hidden focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all text-sm"
        >
        ...
        {targetOptions.map((opt) => (
          <label
            ...
            className={`flex items-center gap-3 px-4 py-3 rounded-xl border cursor-pointer transition-all ${
              targetType === opt.id
                ? "border-blue-500 bg-blue-50/50 dark:bg-blue-950/20 text-blue-700 dark:text-blue-400 font-semibold"
          ...
          >
            <input
              ...
              onChange={() => setTargetType(opt.id as any)}
              disabled={loading}
  ...
  const renderUserSelectionGrid = () => {
    if (targetType !== "specific") return null;

    return (
      <div className="space-y-3 animate-in fade-in slide-in-from-top-2 duration-200 border-t border-slate-100 dark:border-slate-700 pt-4">
        ...
        <div className="relative">
          <input
            ...
            disabled={loading}
            className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-slate-250 dark:border-slate-650 bg-slate-50 dark:bg-slate-900 text-slate-855 dark:text-slate-100 focus:outline-hidden focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all text-sm"
          />
        ...
        <div className="relative border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden bg-slate-50/50 dark:bg-slate-900/50 p-2">
          {fetchingUsers && usersList.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-10 space-y-2">
              <Icon icon="line-md:loading-twotone-loop" className="text-3xl text-blue-500" />
                  ...
                  >
                    <div className="mt-0.5">
                      <Icon
                        ...
                        className={`text-xl ${isSelected ? "text-blue-500" : "text-slate-350 dark:text-slate-650"}`}
                      />
                    </div>
  ...
  const renderSubmitButton = () => (
    <div className="pt-2">
      <button
        type="submit"
        disabled={loading}
        className="w-full py-3 px-4 rounded-xl font-semibold text-white bg-blue-600 hover:bg-blue-700 active:bg-blue-800 disabled:bg-blue-450 dark:disabled:bg-blue-900/50 flex items-center justify-center gap-2 cursor-pointer shadow-md hover:shadow-lg transition-all text-sm"
      >
        {loading ? (
          <>
