  ...
  return (
    <div className="p-6 space-y-6 mx-auto min-h-screen text-slate-800 w-full dark:text-slate-100 transition-colors duration-200">
          ...
          {permissions.update && (
            ...
            >
              {syncing ? (
                ...
                <Icon icon="material-symbols:sync-saved-locally-outline-rounded" className="text-base animate-pulse" />
              )}
              Đồng bộ dữ liệu RAG
            </button>
          )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-100 dark:border-slate-700/50 shadow-xs flex items-center justify-between">
          <div>
            <span className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Tệp tin mẫu</span>
            <a
              href="data:text/csv;charset=utf-8,%EF%BB%BFkeyword,question,content%0Agia%20ca,Gia%20dich%20vu%2520don%252520det%2520la%2520bao%2520nhieu%3F,Chi%20phi%20don%20dep%20nha%20cua%20la%20100.000d%2Fgio.%0Alien%20he,So%20dien%20thoai%20hotline%20la%20gi%3F,Vui%20long%20goi%20hotline%20(+84)%20123-456-789."
          ...
          <div className="w-12 h-12 rounded-xl bg-cyan-50 dark:bg-cyan-950/20 text-cyan-600 dark:text-cyan-400 flex items-center justify-center border border-cyan-100 dark:border-cyan-900/30">
      ...
      {loading ? (
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700/50 p-12 flex flex-col items-center justify-center gap-4">
          <span className="w-10 h-10 border-4 border-slate-200 border-t-[#026E5F] rounded-full animate-spin" />
          <span className="text-sm font-semibold text-slate-500 dark:text-slate-400">Đang tải tri thức chatbot...</span>
        </div>
      ) : knowledges.length === 0 ? (
                      ...
                      <div className="flex items-center justify-end gap-2">
                        ...
                        {permissions.delete && (
                          <button
                            ...
                            className="w-8 h-8 rounded-lg flex items-center justify-center hover:bg-red-50 dark:hover:bg-red-955/20 text-red-650 dark:text-red-500 cursor-pointer transition-colors"
                            title="Xóa tri thức"
                          >
      ...
      {isEditModalOpen && (
        <div className="fixed inset-0 z-200 overflow-y-auto bg-black/60 dark:bg-black/80 flex items-center justify-center p-4 backdrop-blur-xs">
          <form
            onSubmit={handleSave}
            className="relative bg-white dark:bg-slate-800 w-full max-w-lg rounded-2xl shadow-2xl border border-slate-100 dark:border-slate-700 overflow-hidden transform transition-all flex flex-col max-h-[90vh]"
          >
            {/* Modal Header */}
            <div className="p-5 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between bg-slate-50/50 dark:bg-slate-900/10">
              ...
              <button
                ...
                className="text-slate-400 hover:text-slate-655 dark:hover:text-slate-200 cursor-pointer transition-colors"
              >
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4">
              <div className="flex flex-col gap-1">
                ...
                <input
                  ...
                  className="w-full p-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-xs focus:ring-2 focus:ring-[#026E5F]/20 focus:outline-hidden focus:border-[#026E5F] text-slate-700 dark:text-slate-200"
                />
                <span className="text-[10px] text-slate-400">Không bắt buộc. Giúp hệ thống dễ khớp ưu tiên tri thức hơn.</span>
              </div>

              <div className="flex flex-col gap-1">
                <label className="text-xxs font-bold text-slate-450 dark:text-slate-500 uppercase">Câu hỏi tham khảo (Question) *</label>
                <textarea
                  rows={2}
                  required
              ...
              <div className="flex flex-col gap-1">
                <label className="text-xxs font-bold text-slate-450 dark:text-slate-500 uppercase">Nội dung câu trả lời chuẩn (Context Content) *</label>
                <textarea
                  rows={4}
                  required
            ...
            <div className="p-5 bg-slate-50 dark:bg-slate-900/30 border-t border-slate-200 dark:border-slate-700/80 flex items-center justify-end gap-3">
              <button
                type="button"
                onClick={handleCloseModal}
                className="px-4 py-2 border border-slate-205 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-700 text-xs font-bold rounded-xl transition-all cursor-pointer text-slate-705 dark:text-slate-350"
              >
                Hủy bỏ
              </button>
              <button
