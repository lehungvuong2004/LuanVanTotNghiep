    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
      <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-100 dark:border-slate-700/50 shadow-xs flex flex-col">
        <div className="flex justify-between items-center mb-2">
          <h3 className="text-sm font-bold text-slate-800 dark:text-slate-100">Trạng Thái Hồ Sơ</h3>
      ...
      <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-100 dark:border-slate-700/50 shadow-xs flex flex-col">
        <div className="flex justify-between items-center mb-2">
          <h3 className="text-sm font-bold text-slate-800 dark:text-slate-100">Đánh Giá Trung Bình</h3>
    ...
    if (helpers.length === 0) {
      return (
        <div className="flex flex-col items-center justify-center py-16 px-4 bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700/50 shadow-xs text-center">
          ...
          <h3 className="text-lg font-bold text-slate-800 dark:text-slate-200">Không tìm thấy người giúp việc nào</h3>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1 max-w-sm">Hãy thử thay đổi từ khóa tìm kiếm hoặc trạng thái lọc.</p>
        </div>
      );
    }
          ...
          <table className="w-full text-left border-collapse min-w-4xl">
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-900/30 border-b border-slate-200/60 dark:border-slate-700 text-slate-550 dark:text-slate-400 text-xs font-bold uppercase tracking-wider">
                ...
                <th className="py-3 px-5">Trạng Thế Hồ Sơ</th>
              ...
              {helpers.map((helper) => {
                ...
                return (
                  <tr key={helper.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-750/30 transition-colors">
                    ...
                    <td className="py-3 px-5">
                      <span className="font-medium text-slate-650 dark:text-slate-350">{phone}</span>
                    </td>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div className="p-3 bg-white dark:bg-slate-900 border border-slate-150 dark:border-slate-700/80 rounded-xl flex items-center gap-3 shadow-xs">
                ...
                <div>
                  <p className="text-xxs text-slate-400 dark:text-slate-500 font-semibold uppercase tracking-wider">Đánh giá chung</p>
            ...
            {selectedHelper.bio && (
              <div className="space-y-1">
                <h5 className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Tiểu sử (Bio)</h5>
            ...
            <div className="space-y-2">
              <h5 className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Dịch Vụ Có Thể Đảm Nhận ({selectedHelper.skills?.length || 0})</h5>
              <div className="flex flex-wrap gap-2">
                {selectedHelper.skills && selectedHelper.skills.length > 0 ? (
                  ...
                  <span className="text-xs italic text-slate-400 dark:text-slate-600">Chưa khai báo dịch vụ nào</span>
                )}
              </div>
            </div>
            ...
            <div className="space-y-2">
              <h5 className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Lịch Sử Kiểm Duyệt Hồ Sơ</h5>
              <div className="space-y-2 max-h-40 overflow-y-auto pr-1">
                {selectedHelper.verifications && selectedHelper.verifications.length > 0 ? (
                  ...
                  <div className="text-xs italic text-slate-400 dark:text-slate-600">Chưa có lịch sử phê duyệt</div>
                )}
              </div>
            </div>
          </div>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div className="relative bg-white dark:bg-slate-800 w-full max-w-md rounded-2xl shadow-2xl border border-slate-100 dark:border-slate-700 overflow-hidden transform transition-all duration-300 scale-100 flex flex-col">
          ...
          <div className="p-6 space-y-4">
            ...
            <div className="space-y-1">
              <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Ghi chú (Note)</label>
              <textarea
                value={verifyNote}
            ...
            <div className="flex items-center justify-end gap-3 pt-3 border-t border-slate-200 dark:border-slate-700/80 mt-4">
              ...
              <button
                ...
                className={`flex items-center justify-center gap-2 text-white font-bold px-5 py-2.5 rounded-xl text-sm shadow-md active:scale-98 transition-all cursor-pointer ${
                  verifyStatus === "approved" ? "bg-emerald-600 hover:bg-emerald-700" : "bg-rose-600 hover:bg-rose-700"
                }`}
              >
                Xác nhận
              </button>
        ...
        <div className="relative bg-white dark:bg-slate-800 w-full max-w-md rounded-2xl shadow-2xl border border-slate-100 dark:border-slate-700 overflow-hidden transform transition-all duration-300 scale-100 flex flex-col">
          ...
          <div className="p-6 space-y-4">
            <p className="text-xs font-semibold text-slate-500 dark:text-slate-400">
              Đang chỉnh sửa trạng thái đối tác: <span className="font-extrabold text-[#026E5F]">{name}</span>
            </p>
      ...
      {selectedHelperIds.length > 0 && (
        <div className="my-2">
          <BulkDeleteBar
            ...
            onDeleteSelected={handleBulkDeleteHelpers}
            onClear={() => setSelectedHelperIds([])}
            loading={loading}
          />
        </div>
      )}
