...
import type { Permission } from "../../../api/rolesApi/roles";
...
export const PermissionsMatrix = () => {
  ...
  const renderRoleTabs = () => (
    <div className="flex flex-wrap items-center gap-2.5">
      <span className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mr-2">VAI TRÒ:</span>
      {roles.map((r) => (
        <button
          key={r.id}
          ...
          className={`px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider transition-all cursor-pointer ${
            selectedRoleId === r.id
              ? "bg-cyan-900 text-white border border-cyan-900 shadow-xs"
              : "bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-650 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700"
          }`}
        >
          {r.name}
        </button>
  ...
  const renderStatsCards = () => (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
      <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl shadow-xs flex flex-col justify-between h-28">
      ...
      <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl shadow-xs flex flex-col justify-between h-28 relative overflow-hidden">
  ...
  const renderMatrix = () => (
    <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-150 dark:border-slate-750 shadow-xs overflow-hidden flex flex-col flex-1">
      <div className="p-5 border-b border-slate-100 dark:border-slate-700/60 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-50/20 dark:bg-slate-900/10">
      ...
      <div className="overflow-x-auto overflow-y-auto flex-1 custom-scrollbar">
        <div className="w-full text-sm min-w-4xl flex flex-col">
          <div className="grid grid-cols-11 min-w-252 bg-slate-50/40 dark:bg-slate-900/30 border-b border-slate-100 dark:border-slate-800 text-xxs font-extrabold text-slate-400 dark:text-slate-500 uppercase tracking-widest text-left">
            <div className="col-span-3 px-6 py-4 sticky left-0 z-20 bg-slate-50/90 dark:bg-slate-900/90 shadow-sm border-r border-slate-100/50 dark:border-slate-800/40 flex items-center">
              MODULE NAME
            </div>
              ...
              filteredModulesList.map((mod) => {
                ...
                return (
                  <div key={mod} className="group grid grid-cols-11 min-w-252 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                    <div className="col-span-3 px-6 py-3.5 sticky left-0 z-15 bg-white group-hover:bg-slate-50 dark:bg-slate-850 dark:group-hover:bg-slate-800/50 shadow-sm font-bold text-slate-800 dark:text-slate-200 border-r border-slate-100/50 dark:border-slate-800/40 flex items-center gap-3 transition-colors">
                      ...
                      if (!perm) {
                        return (
                          <div key={col.key} className="col-span-1 px-4 py-3.5 flex items-center justify-center">
                            <div
                              className="w-5 h-5 rounded border border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800/50 cursor-not-allowed"
