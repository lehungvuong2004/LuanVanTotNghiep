...
export const Reports = () => {
          ...
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-bold border transition cursor-pointer ${
              statusFilter === status
                ...
                : "bg-transparent border-slate-200 dark:border-slate-700 text-slate-650 dark:text-slate-350 hover:bg-slate-50 dark:hover:bg-slate-800"
            }`}
          >
                ...
                return (
                  <tr key={report.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition duration-150">
                    ...
                    <td className="px-6 py-4.5 whitespace-nowrap">
                      <span className="text-xs font-extrabold text-slate-455 dark:text-slate-500">#{report.id}</span>
                    </td>
                    <td className="px-6 py-4.5 whitespace-nowrap">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col text-left">
                          ...
                          <div className="flex items-center gap-1.5 mt-0.5">
                            ...
                            <span className="text-xs text-slate-455 dark:text-slate-500">ID: {report.report_by}</span>
                          </div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4.5 whitespace-nowrap">
                      <div className="flex items-center gap-3">
                        {reported?.avatar ? (
                          ...
                          <div className="w-9 h-9 rounded-full bg-rose-50 dark:bg-rose-955/30 text-rose-650 dark:text-rose-455 flex items-center justify-center font-bold text-sm">
                        ...
                        <div className="flex flex-col text-left">
                          ...
                          <div className="flex items-center gap-1.5 mt-0.5">
                            ...
                            <span className="text-xs text-slate-455 dark:text-slate-500">ID: {report.reported_user_id || "N/A"}</span>
                          </div>
                        </div>
                      </div>
                    </td>
                      ...
                      <div className="flex items-center justify-center gap-2">
                        ...
                        {permissions.delete && (
                          <button
                            ...
                            title="Xóa báo cáo"
                            className="p-1.5 hover:bg-rose-50 dark:hover:bg-rose-955/20 text-rose-650 dark:text-rose-455 rounded-lg transition-colors cursor-pointer"
                          >
  ...
  const renderDetailModal = () => {
    if (!selectedReport || isProcessOpen) return null;
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-fade-in">
        <div className="bg-white dark:bg-slate-850 w-full max-w-xl rounded-3xl shadow-2xl border border-slate-150 dark:border-slate-700/60 overflow-hidden flex flex-col transform transition-all duration-300">
          {/* Header */}
          <div className="flex items-center justify-between px-6 py-4.5 border-b border-slate-100 dark:border-slate-700/50">
            <div className="text-left">
              ...
              <p className="text-xs text-slate-450 mt-0.5">Mã báo cáo: #{selectedReport.id}</p>
            </div>
            <button
              onClick={handleCloseDetail}
              className="w-9 h-9 rounded-xl flex items-center justify-center bg-slate-50 dark:bg-slate-800 text-slate-400 hover:text-slate-650 dark:hover:text-slate-200 hover:scale-105 transition cursor-pointer"
            >
          ...
          <div className="px-6 pb-6 flex gap-3 border-t border-slate-100 dark:border-slate-800/50 pt-4.5">
            <button
              onClick={handleCloseDetail}
              className="flex-1 px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 text-slate-655 dark:text-slate-300 text-sm font-bold hover:bg-slate-50 dark:hover:bg-slate-800 transition cursor-pointer"
            >
              Đóng
            </button>
            {permissions.process && selectedReport.status === "pending" && (
              <>
                ...
                <button
                  ...
                  className="px-6 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-bold transition active:scale-95 cursor-pointer shadow-sm"
                >
                  Giải quyết
                </button>
              </>
    ...
    return (
      <div className="fixed inset-0 z-55 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-fade-in">
        <form
          ...
          className="bg-white dark:bg-slate-850 w-full max-w-md rounded-3xl shadow-2xl border border-slate-150 dark:border-slate-700/60 overflow-hidden flex flex-col transform transition-all duration-300"
        >
          {/* Header */}
          <div className="flex items-center justify-between px-6 py-4.5 border-b border-slate-100 dark:border-slate-700/50 text-left">
            <div>
              ...
              <p className="text-xs text-slate-450 mt-0.5">Xử lý báo cáo vi phạm #{selectedReport?.id}</p>
            </div>
            <button
              type="button"
              onClick={handleCloseProcess}
              className="w-9 h-9 rounded-xl flex items-center justify-center bg-slate-50 dark:bg-slate-800 text-slate-400 hover:text-slate-655 dark:hover:text-slate-200 hover:scale-105 transition cursor-pointer"
            >
              <Icon icon="material-symbols:close" className="text-xl" />
            </button>
          </div>
          ...
          <div className="p-6 space-y-4 text-left">
            <div>
              <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">
                Ghi chú xử lý <span className="text-slate-450 font-normal">(không bắt buộc)</span>
              </label>
              <textarea
                value={processNote}
          ...
          <div className="px-6 pb-6 flex gap-3">
            ...
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-white text-sm font-bold transition active:scale-95 cursor-pointer shadow-sm ${
                processStatus === "resolved" ? "bg-emerald-600 hover:bg-emerald-700" : "bg-slate-500 hover:bg-slate-600"
              }`}
            >
