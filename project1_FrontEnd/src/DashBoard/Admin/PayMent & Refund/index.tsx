...
export const PaymentsRefunds = ({ defaultTab = "payments" }: PaymentsRefundsProps) => {
  const {
    activeTab,
    setActiveTab,
    loading,
  ...
  const renderHeader = () => {
    return (
      <div>
        <h1 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-white">Quản lý Thanh toán & Hoàn tiền</h1>
        <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Theo dõi giao dịch, xử lý hoàn tiền và kiểm tra doanh thu theo thời gian thực.</p>
      </div>
    );
  };
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="flex items-center gap-4 rounded-xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-950 p-5 shadow-sm">
          ...
          <div className="min-w-0 flex-1">
            ...
            <h3 className="mt-1 text-xl font-bold text-slate-800 dark:text-slate-100 truncate">{computedMetrics.pendingRefundsCount} yêu cầu</h3>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 rounded-xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-950 p-5 shadow-sm">
          ...
          <div className="min-w-0 flex-1">
            ...
            <h3 className="mt-1 text-xl font-bold text-slate-800 dark:text-slate-100 truncate">{fmtVND(computedMetrics.completedRefundsAmount)}</h3>
          </div>
        </div>
      </div>
    );
            ...
            {(["payments", "refunds"] as const).map((tab) => (
              <button
                ...
                className={`flex items-center gap-1.5 px-4 py-2 text-sm font-semibold rounded-md transition-all cursor-pointer ${
                  activeTab === tab ? "bg-white dark:bg-slate-950 text-[#066d72] dark:text-teal-400 shadow-sm" : "text-slate-500 hover:text-slate-700 dark:hover:text-slate-300"
                }`}
              >
          ...
          {activeTab === "payments" ? (
            <>
              ...
              {["All", "cash", "vnpay"].map((m) => (
                ...
                >
                  {m === "All" ? "Tất cả phương thức" : (PAYMENT_METHODS_LABELS[m] ?? m)}
                </button>
              ))}
            </>
          ) : (
