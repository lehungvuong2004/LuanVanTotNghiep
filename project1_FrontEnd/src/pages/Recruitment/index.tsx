...
const HeroSection = ({ t }: HeroProps) => (
  <div className="relative overflow-hidden rounded-3xl bg-linear-to-br from-teal-700 via-teal-650 to-emerald-650 dark:from-teal-900 dark:via-teal-850 dark:to-emerald-900 text-white pt-12 pb-16 px-6 md:px-14 shadow-xl">
    {/* decorative blobs */}
            ...
            <button
              ...
              className={`px-3.5 py-1.5 rounded-full border text-xs font-semibold transition-all cursor-pointer ${
                selectedUrgency === opt.value
                  ...
                  : "border-slate-200 dark:border-slate-700 text-slate-550 dark:text-slate-400 hover:border-[#026E5F] hover:text-[#026E5F]"
              }`}
            >
              {t(opt.label)}
            </button>
...
const JobCard = ({ t, job }: JobCardProps) => {
  ...
  return (
    <article className="relative flex flex-col bg-white dark:bg-slate-800 rounded-2xl border border-slate-150 dark:border-slate-700/50 overflow-hidden shadow-sm hover:shadow-xl hover:-translate-y-1 hover:border-[#026E5F] dark:hover:border-[#026E5F]/50 transition-all duration-300 group">
      <div className="flex flex-col flex-grow p-6">
        ...
        <div className="grid grid-cols-2 gap-x-3 gap-y-2 pt-4 border-t border-slate-100 dark:border-slate-700/50 mb-5">
          ...
          <div className="flex items-center gap-1.5 text-xs text-slate-400 dark:text-slate-550 col-span-2">
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700/50 px-5 py-3.5 shadow-sm">
        <div className="relative flex-grow max-w-sm">
          ...
          <input
            ...
            className="pl-10 pr-4 py-2 w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-sm text-slate-755 dark:text-slate-200 placeholder-slate-400 focus:outline-none focus:border-[#026E5F] focus:ring-1 focus:ring-[#026E5F] transition"
          />
        </div>
        <div className="flex items-center gap-3 shrink-0">
          <span className="text-xs text-slate-450 dark:text-slate-500 hidden sm:block font-medium">
      ...
      {jobs.length > 0 ? (
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700/50 p-12 text-center shadow-sm">
          ...
          <h3 className="text-lg font-bold text-slate-705 dark:text-slate-300">{t("Không tìm thấy kết quả phù hợp")}</h3>
