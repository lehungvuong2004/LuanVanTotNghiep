...
export const Pricing = () => {
  ...
  const renderHeaderTitle = () => (
    <div className="mt-10 mb-6 flex flex-col items-center">
      <h2 className="text-2xl md:text-3xl font-extrabold text-[#026E5F] dark:text-teal-400 tracking-tight">
        {t("Bảng giá dịch vụ")}
      </h2>
    </div>
  );
      ...
      <div className="flex flex-col gap-12">
        {pricingCategories.map((category) => (
          <div key={category.id} className="flex flex-col gap-4">
            {category.name && (
              <div className="flex items-center gap-3 px-2 py-1 border-b border-slate-100 dark:border-slate-800/80">
                {category.icon && (
                  <div className="w-10 h-10 rounded-xl bg-teal-50 dark:bg-teal-950/40 text-[#026E5F] dark:text-teal-400 flex items-center justify-center">
                    <Icon icon={category.icon} className="text-xl" />
                  </div>
                )}
                <h3 className="text-lg md:text-xl font-extrabold text-slate-850 dark:text-white">
                  {category.name}
                </h3>
              </div>
            )}
              ...
              <table className="w-full text-left border-collapse min-w-max">
                <thead>
                  <tr className="bg-slate-50 dark:bg-slate-800/80">
                    <th className="px-6 py-5 text-sm font-bold text-slate-750 dark:text-white border-b border-r last:border-r-0 border-slate-200 dark:border-slate-700 w-60">
                      {t("Gói Dịch Vụ")}
                    </th>
