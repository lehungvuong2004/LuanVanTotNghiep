  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700/50 p-6 shadow-sm sticky top-24 flex flex-col gap-6">
      <div className="flex items-center justify-between pb-4 border-b border-slate-100 dark:border-slate-700/50">
        ...
        <button className="text-xs text-teal-600 dark:text-teal-400 font-semibold hover:underline cursor-pointer">{t("Xóa bộ lọc")}</button>
      </div>
        ...
        <div className="flex flex-col gap-2.5">
          {categories.map((cat, index) => (
            <label key={cat} className="flex items-center gap-3 cursor-pointer group">
              <input type="checkbox" defaultChecked={index === 0} className="rounded border-slate-300 dark:border-slate-700 text-teal-600 focus:ring-teal-500 dark:bg-slate-900 cursor-pointer" />
              <span className="text-sm text-slate-600 dark:text-slate-300 group-hover:text-slate-800 dark:group-hover:text-white transition-colors font-medium">{t(cat)}</span>
            </label>
          ))}
        </div>
      </div>
      ...
      <div>
        <div className="flex justify-between items-center mb-3">
          ...
          <span className="text-xs font-bold text-teal-600 dark:text-teal-400">2.000.000đ</span>
        </div>
        <div className="space-y-2">
          <input type="range" min="0" max="2000000" step="50000" defaultValue="2000000" className="w-full accent-teal-600 dark:accent-teal-400 cursor-pointer" />
        ...
        <div className="flex flex-col gap-2">
          {ratings.map((r, index) => (
            <label key={r} className="flex items-center gap-3 cursor-pointer group">
              <input type="radio" name="rating" defaultChecked={index === 0} className="border-slate-300 dark:border-slate-700 text-teal-600 focus:ring-teal-500 dark:bg-slate-900 cursor-pointer" />
          ...
          {tabs.map((tab, index) => (
            <button
              ...
              className={`px-5 py-2.5 rounded-full text-sm font-semibold whitespace-nowrap transition-all duration-200 cursor-pointer ${
                index === 0 ? "bg-teal-600 text-white shadow-md shadow-teal-600/10" : "hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300"
              }`}
            >
              {t(tab)}
            </button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {services.slice(0, visibleCount).map((service) => (
              ...
              >
                ...
                <div className="aspect-[4/3] overflow-hidden bg-slate-100 dark:bg-slate-900">{renderServiceImage(service.image, service.title)}</div>
                  ...
                  <div className="mt-auto pt-4 border-t border-slate-100 dark:border-slate-700/50 flex items-center justify-between">
                    <div className="flex flex-col">
                      ...
                      <div className="flex items-baseline gap-0.5">
                        <span className="text-xl font-bold text-teal-600 dark:text-teal-400">{service.price.toLocaleString("vi-VN")}đ</span>
...
const FeaturedHelpers = ({ t, helpers }: FeaturedHelpersProps) => {
  const renderHelperAvatar = (url?: string, name?: string) => {
    ...
    return <div className="w-full h-full bg-slate-200 dark:bg-slate-700 flex items-center justify-center text-slate-500 dark:text-slate-400 font-bold text-lg">{name ? name.charAt(0) : "H"}</div>;
  };
