...
export const getAddUserSchema = (t: (key: string) => string = (key) => key) =>
  Yup.object().shape({
    full_name: Yup.string().min(2, t("Họ tên phải có ít nhất 2 ký tự")).max(100, t("Họ tên không được vượt quá 100 ký tự")).required(t("Vui lòng nhập họ tên")),
    email: Yup.string().email(t("Định dạng email không hợp lệ")).required(t("Vui lòng nhập email")),
    phone: Yup.string()
      .matches(/^(0[3|5|7|8|9])[0-9]{8}$/, t("Số điện thoại không hợp lệ (10 số, bắt đầu bằng 03, 05, 07, 08, 09)"))
      .nullable(),
    role_id: Yup.number().oneOf([ROLES.ADMIN, ROLES.OPERATOR, ROLES.HELPER, ROLES.CUSTOMER], t("Vai trò không hợp lệ")).required(t("Vui lòng chọn vai trò")),
    password: Yup.string()
