...
export const getRegisterSchema = (t: any) =>
  Yup.object({
    fullName: Yup.string()
      ...
      .max(50, t("Họ và tên không được vượt quá 50 ký tự")),
    email: Yup.string()
      .required(t("Vui lòng nhập email"))
      .email(t("Vui lòng nhập đúng định dạng email")),
    phone: Yup.string()
      ...
      .matches(
        /^(0[3|5|7|8|9])[0-9]{8}$/,
        t("Số điện thoại không hợp lệ (10 số, bắt đầu bằng 03, 05, 07, 08, 09)")
      ),
    password: Yup.string()
      .required(t("Vui lòng nhập mật khẩu"))
      .min(6, t("Mật khẩu phải có ít nhất 6 ký tự"))
    ...
    agreeTerms: Yup.boolean().oneOf(
      [true],
      t("Bạn phải đồng ý với điều khoản sử dụng và chính sách bảo mật")
    ),
  });
...
export const getForgotPasswordStep1Schema = (t: any) =>
  Yup.object({
    emailOrPhone: Yup.string()
      ...
      .test(
        "is-email",
        t("Vui lòng nhập đúng định dạng Email (vd: example@gmail.com)"),
        (value) => {
