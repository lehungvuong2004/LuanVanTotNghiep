...
export const createRoleAdmin = async (data: { name: string; description?: string }): Promise<Role> => {
  const response = await axiosInstance.post<Role>("/admin/roles", data);
  return response.data;
};

export const updateRoleAdmin = async (id: number, data: { name?: string; description?: string }): Promise<Role> => {
